...
public class ThreeSumBenchmark {
	...
	public static void main(String[] args) {
		new ThreeSumBenchmark(200, 125, 125).runBenchmarks();
...
//      long startTime = System.nanoTime();
		...
		double timer= bt.runFromSupplier(supplier, runs);
		
		timeLoggers[0].log(timer, n);
		timeLoggers[1].log(timer, n);
...
//      timeLoggers[0].log((double) (endTime - startTime), n);
//      timeLoggers[1].log((double) (endTime - startTime), n);
		// END
	}
	...
	private final static TimeLogger[] timeLoggersQuadrithmic = {
			new TimeLogger("Raw time per run (mSec): ", (time, n) -> time), new TimeLogger(
					"Normalized time per run (n^2 log n): ", (time, n) -> time / n / n / Utilities.lg(n) * 1e6) };
